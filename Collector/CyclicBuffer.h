...
public:
	...
	CyclicBuffer& Write(const PROCESS_BUFFER* data, ULONG size);
...
template<typename TLock>
CyclicBuffer<TLock>& CyclicBuffer<TLock>::Write(const PROCESS_BUFFER* data, ULONG size) {
	AutoLock<TLock> locker(_lock);
	if (_buffer == nullptr)
		return *this;
	...
	if (_currentWriteOffset + size > _maxSize) {
		if (_currentWriteOffset + sizeof(PROCESS_BUFFER) <= _maxSize) {
			memset(_buffer + _currentWriteOffset, 0, sizeof(PROCESS_BUFFER));
		}
...
ULONG CyclicBuffer<TLock>::Read(PUCHAR target, ULONG size) {
	...
	while (total < size && _itemCount > 0) {
		auto header = reinterpret_cast<PROCESS_BUFFER*>(_buffer + _currentReadOffset);
		...
		NT_ASSERT(itemSize >= sizeof(PROCESS_BUFFER));
		...
		if (_currentReadOffset + sizeof(PROCESS_BUFFER) > _maxSize) {
			_currentReadOffset = 0;
		}
		else if (_itemCount > 0) {
			header = reinterpret_cast<PROCESS_BUFFER*>(_currentReadOffset + _buffer);
			if (header->Size == 0)
				_currentReadOffset = 0;
		}
	}
